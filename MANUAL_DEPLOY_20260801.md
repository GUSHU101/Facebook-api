...
1. 打开本项目 `/admin`，分别为两个店铺复制最新生成的 `shopify-pixel-v17` 代码。
2. Shopify 后台进入 `Settings -> Customer events`。
3. 在 Custom Pixel 隐私设置中如实声明 Analytics、Marketing 与 Data sale；v17 会读取初始授权并监听后续变化。
4. 每店只保留一套本项目自定义像素，替换旧 v14，不要并行启用 v14、v15、v16 与 v17。
...
8. v17 首个真实事件进入网关后，店铺表中的“像素运行版本”应显示 `shopify-pixel-v17` 和最近事件时间；“浏览器投递丢失诊断”正常应为空。
